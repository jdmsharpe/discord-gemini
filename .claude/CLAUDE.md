...
- Default video model is `gemini-omni-flash-preview`, generated via the **Interactions API** (`_generate_video_with_omni` in `video.py`), NOT the Veo `generate_videos` path: the interaction completes synchronously and returns a file URI, which is downloaded via `files.download`. Veo 3.1 models remain for image-to-video, last-frame interpolation, and resolution/duration control. Omni is billed per video-modality OUTPUT token (`VIDEO_TOKEN_PRICING` / `calculate_omni_video_cost`, exact from the response's usage), while Veo is per-second-by-resolution (`VIDEO_PRICING`, estimated). Veo-only options are rejected for Omni via `_validate_omni_video_request`. Keep the default model, `discord_gemini.util.VIDEO_PRICING`/`VIDEO_TOKEN_PRICING`, the `/gemini-media video` slash-command metadata, and user-facing docs aligned when changing it.
...
- **SDK migration triggers**: pinned to `google-genai ~=2.19`, **locked/running 2.19.0**. Veo generation calls `generate_videos(model=..., source=types.GenerateVideosSource(...), config=...)` — the `prompt`/`image`/`video` arguments were deprecated in 2.14.0 and slated for removal no earlier than 2026-07-31; `last_frame` stays a `GenerateVideosConfig` field, not a source field. `_build_veo_image` must keep wrapping raw bytes in `types.Image`: a `PIL.Image` is silently coerced to an all-`None` `types.Image` rather than raising. **Both rechecked at 2.19.0: `prompt`/`image`/`video` are STILL present (removal date passed without action, and we use `source=` regardless), and the PIL coercion STILL happens — `GenerateVideosConfig(last_frame=<PIL>)` yields a bare `Image()` with no bytes, so `_build_veo_image` remains load-bearing.** Re-check both on the next bump rather than blind-bumping the pin. 2.18.1 also REMOVED the `Turn`/`TurnContent` interactions types and dropped `List[Turn]` from `InteractionsInput` — inert here only because the Omni video and research flows pass kwargs to `interactions.create` and never name `Turn`; both are STILL absent at 2.19.0, so re-check if either flow starts building typed turns. Its one-shot AFC warning in `Models.generate_content` no longer fires: every request config sets `client.disable_afc()`, so `generate_content` skips the SDK's AFC wrapper entirely. Through v1.10.0 the flag was set only when custom functions were enabled, and the warning tripped `AsyncModels._logged_afc_warning` once per process.
