...
- Default chat model is `gemini-3.6-flash`; keep the `/gemini chat` `model` param default (`cogs/gemini/cog.py`), `CHAT_MODEL_CHOICES` ordering (`cogs/gemini/command_options.py`), the tool `model_allowlist`s in `cogs/gemini/tool_registry.py`, the `models` block in `config/pricing.yaml`, and `CACHE_MIN_TOKEN_COUNT` (`util.py`) aligned when changing it. `MODEL_PRICING.get(model, UNKNOWN_CHAT_MODEL_PRICING)` falls back silently, so an unpriced model bills at the fallback rate with no error.
...
- **SDK migration triggers**: pinned to `google-genai ~=2.14`. Veo generation calls `generate_videos(model=..., source=types.GenerateVideosSource(...), config=...)` — the `prompt`/`image`/`video` arguments were deprecated in 2.14.0 and are slated for removal no earlier than 2026-07-31; `last_frame` stays a `GenerateVideosConfig` field, not a source field. `_build_veo_image` must keep wrapping raw bytes in `types.Image`: a `PIL.Image` is still silently coerced to an all-`None` `types.Image` in 2.14.0. Re-check both against the release notes on the next bump rather than blind-bumping the pin.
